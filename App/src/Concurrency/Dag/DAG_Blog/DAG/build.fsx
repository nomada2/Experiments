...
let reportFsiError (e:exn) =
  traceError "Reloading dag.fsx script failed."
...
let reloadScript () =
  try
    ...
    let appFsx = __SOURCE_DIRECTORY__ @@ "dag.fsx"
    fsiSession.EvalInteraction(sprintf "#load @\"%s\"" appFsx)
    fsiSession.EvalInteraction("open Dag")    
  with e -> reportFsiError e; 


Target "run" (fun _ ->
  // Watch for changes & reload when dag.fsx changes
  use watcher = !! (__SOURCE_DIRECTORY__ @@ "*.*") |> WatchChanges(fun change -> reloadScript())
  traceImportant "Waiting for dagmodule.fs edits. Press any key to stop."
